...
const HomeMidSection: FC<{ lang: string }> = ({ lang }) => {
  const isArabicLang = lang === LocalesLanguage.AR;
  const arabicLangTextStyle = isArabicLang ? "text-right" : "";
  ...
  return (
    <div className="flex flex-col-reverse sm:flex-row w-full h-full">
      ...
      <div className="flex flex-col  items-left justify-center gap-4 p-10 bg-black w-full">
        <p
          className={`text-white ${arabicLangTextStyle} font-bold text-4xl border-b-2 border-red-500`}
        >
          {locale[lang].our_stores}
        </p>
        <p className={`text-2xl ${arabicLangTextStyle} text-gray-400`}>
          {locale[lang].high_performance}
        </p>
        <p className={`text-gray-400 ${arabicLangTextStyle} text-sm font-sans`}>
